...
operators: Dict[int, Callable[[List[int]], int]] = {
    0: sum,
    1: lambda children: reduce(lambda a, b: a * b, children, 1),
    2: min,
    3: max,
...
class Operator(Package):
    ...
    def get_version(self) -> int:
        return self.version + sum([child.get_version() for child in self.children])

    def evaluate(self) -> int:
        children = [child.evaluate() for child in self.children]

        return operators.get(self.type_, lambda _: 0)(children)
